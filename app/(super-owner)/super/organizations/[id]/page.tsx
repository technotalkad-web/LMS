    ...
    <div className="max-w-4xl">
      ...
      <header className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight text-ink flex items-center gap-3">
          <Building2 className="w-7 h-7 text-accent" /> {o.name}
        </h1>
        <p className="text-muted mt-1 text-sm font-mono">/{o.slug}</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
        <Stat
          label="Plan"
          value={planObj?.name ?? "—"}
          accent={planObj ? "text-accent" : "text-muted"}
        />
        <Stat
          label="Billing"
        ...
        />
        <Stat label="Members" value={totalMembers.toString()} accent="text-ink" />
      </div>
