    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="bg-ink text-canvas hover:opacity-90 px-4 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 transition disabled:opacity-60"
          disabled={busy}
        >
...
function PlanCard({
  ...
  if (!editing) {
    return (
      <div className="bg-paper border border-line rounded-xl shadow-sm p-6 flex flex-col">
        <div className="flex items-baseline justify-between mb-1">
          <h2 className="text-xl font-bold">{plan.name}</h2>
          <span className="text-xs font-mono text-muted">{plan.slug}</span>
        </div>
        <p className="text-3xl font-black mt-2">
          ${Math.round(plan.monthly_price_cents / 100).toLocaleString()}
          <span className="text-sm font-medium text-muted">/mo</span>
        </p>
        <dl className="text-sm text-muted mt-4 space-y-1.5">
        ...
        <ul className="mt-4 space-y-1 text-xs">
          {Object.entries(plan.features ?? {}).map(([k, v]) => (
            <li key={k} className={`flex items-center gap-2 ${v ? "text-emerald-700" : "text-muted line-through"}`}>
              <span>{v ? "✓" : "·"}</span>
              {k.replace(/_/g, " ")}
            </li>
          ))}
        </ul>
        <div className="mt-auto pt-5 border-t border-line flex justify-between items-center text-xs text-muted">
          <span>
            <span className="font-semibold text-ink">{counts.active}</span> active / {counts.total} total
          </span>
          <div className="flex gap-2">
            <button onClick={onEdit} disabled={disabled} className="text-accent font-semibold hover:underline">
              Edit
            </button>
  ...
  return (
    <div className="bg-paper border-2 border-accent rounded-xl shadow-md p-5 flex flex-col">
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <label className="text-xs">
          <span className="text-muted">Price ($/mo)</span>
          <input
            type="number"
      ...
      <div className="border-t border-line pt-3">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {KNOWN_FEATURES.map((k) => (
            <label key={k} className="flex items-center gap-2 text-xs text-ink">
              <input
                type="checkbox"
      ...
      <div className="mt-auto pt-4 flex justify-end gap-2">
        <button onClick={onCancel} disabled={disabled} className="text-muted px-3 py-1.5 rounded-md text-sm hover:bg-canvas flex items-center gap-1">
          <X className="w-3.5 h-3.5" /> Cancel
        </button>
        <button onClick={() => onSave(draft)} disabled={disabled || !draft.name || !draft.slug} className="bg-ink text-canvas hover:opacity-90 px-3 py-1.5 rounded-md text-sm font-semibold flex items-center gap-1 disabled:opacity-60">
    ...
    <label className="text-xs">
      ...
      <div className="flex gap-1 mt-0.5">
        ...
        <button
          ...
          className="px-2 text-xs text-muted hover:text-ink border border-line rounded-md"
          title="Set unlimited"
        >
          ∞
        </button>
